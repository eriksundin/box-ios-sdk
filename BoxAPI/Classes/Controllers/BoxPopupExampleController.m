...
-(NSString*)suggestedFileName {
	NSLog(@"****************getting suggested file name with selection %d", selection);
	switch (selection) {
		case BoxPopupControllerTestButtonImageWithBaseFileAndDate:
...
-(IBAction) popupBoxAction:(id)sender
{
	...
	NSLog(@"************selection is set!");
	// build the needed info and send it to the delegate
	NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:[self suggestedFileName], @"filename",
						  [NSNumber numberWithInt:[self shouldAppendTimeAndDate]], @"appendDateTime",
						  [self data], @"data", [self fileExtension], @"fileExtension", nil];
	[_delegate finishedSelectionWithInfo:info];
